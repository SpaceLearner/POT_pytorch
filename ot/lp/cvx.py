...
def barycenter(A, M, weights=None, verbose=False, log=False, solver='interior-point'):
    ...
    for i in range(n_distributions):
        c = torch.cat((c, M.ravel() * weights[i]))
        b_eq1 = torch.cat((b_eq1, A[:, i]))
    c = torch.cat((c, np.zeros(n)))

    lst_idiag1 = [sps.kron(sps.eye(n), torch.ones((1, n))) for i in range(n_distributions)]
    #  row constraints
    ...
    for i in range(n_distributions):
        if i == 0:
            lst_idiag2.append(sps.kron(torch.ones((1, n)), sps.eye(n)))
            lst_eye.append(-sps.eye(n))
        else:
            lst_idiag2.append(sps.kron(torch.ones((1, n)), sps.eye(n - 1, n)))
            lst_eye.append(-sps.eye(n - 1, n))

    A_eq2 = sps.hstack((sps.block_diag(lst_idiag2), sps.vstack(lst_eye)))
    b_eq2 = torch.zeros((A_eq2.shape[0]))

    # full problem
    A_eq = sps.vstack((A_eq1, A_eq2))
    b_eq = torch.cat((b_eq1, b_eq2))
    ...
    if not cvxopt or solver in ['interior-point']:
        ...
        h = torch.zeros((n_distributions * n2 + n))
